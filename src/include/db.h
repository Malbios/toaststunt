...
static inline bool is_list_of_objs(const Var v, bool include_waifs = false) {
    int i = 0;
    ...
    for (i = 1; i <= length; i++) {
        const auto var_type = v.v.list[i].type;

        if (include_waifs && var_type == TYPE_WAIF)
            continue;

        if (var_type != TYPE_OBJ)
            return false;
    }
...
static inline bool is_obj_or_list_of_objs(const Var v, bool include_waifs = false) {
...
extern int db_object_allows(Var obj, Objid progr, db_object_flag flag);
...
typedef enum { PF_READ = 01,
               PF_WRITE = 02,
               PF_CHOWN = 04 } db_prop_flag;

extern int db_add_propdef(Var obj, const char *pname, Var value, Objid owner,
                          unsigned flags);
...
extern int db_rename_propdef(Var obj, const char *old, const char *_new);
...
extern int db_for_all_propdefs(Var, int (*)(void *, const char *), void *);
...
extern int db_for_all_propvals(Var, int (*)(void *, Var), void *);
...
extern db_prop_handle db_find_property(Var obj, const char *name, Var *value);
...
typedef enum { ASPEC_NONE,
               ASPEC_ANY,
               ASPEC_THIS } db_arg_spec;
...
extern int db_add_verb(Var obj, const char *vnames, Objid owner, unsigned flags,
...
extern int db_for_all_verbs(Var, int (*)(void *, const char *), void *);
