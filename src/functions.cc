...
void register_function_with_read_write(const char *name, int minargs, int maxargs,
   ...
   call_bi_func will free arglist */
{
    ...
    if (func_pc == 1) { /* check arg types and count *ONLY* for first entry */
         ...
         */
        if (args[0].v.num < f.minargs || (f.maxargs != -1 && args[0].v.num > f.maxargs)) {
        ...
        for (k = 0; k < max; k++) {
            ...
            if (!(proto == TYPE_ANY || (proto == TYPE_NUMERIC && (arg == TYPE_INT || arg == TYPE_FLOAT)) || proto == arg)) {
                free_var(arglist);
...
void write_bi_func_data(void *vdata, Byte f_id) {
    const auto functionCount = bf_table.size();
...
int read_bi_func_data(Byte f_id, void **bi_func_state, Byte *bi_func_pc) {
    ...
    if (f_id >= functionCount) {
        ...
        *bi_func_state = nullptr;
         ...
         */
        if (*bi_func_pc == 2 && dbio_input_version == DBV_Float && strcmp(bf_table[f_id].name, "eval") != 0) {
...
load_server_protect_function_flags(void) {
    const auto functionCount = bf_table.size();
    for (size_t i = 0; i < functionCount; i++) {
        bf_table[i]._protected = server_flag_option(bf_table[i].protect_str, 0);
    }
...
void load_server_options(void) {
    int value;

    load_server_protect_function_flags();

#define _BP_DO(PROPERTY, property) \
    _server_int_option_cache[SVO_PROTECT_##PROPERTY] = server_flag_option("protect_" #property, 0);
