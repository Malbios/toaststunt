...
dequeue_input_task(tqueue *tq, enum dequeue_how how)
{
    ...
    if (t)
    {
        *pt = t->next;
        ...
        if (tq->input_suspended && tq->connected && tq->total_input_length < INPUT_LOWAT)
        {
    ...
    else
    {
        ...
        if (!do_intrinsic_command(tq, pc))
        {
            ...
            if (run_server_task_setting_id(tq->player, Var::new_obj(tq->handler),
                                           ...
                                           &result, &(tq->last_input_task_id)) != OUTCOME_DONE ||
                is_true(result))
            {
                /* Do nothing more; we assume :do_command handled it. */
            }
            else if (find_verb_on(_this = tq->player, pc, &vh) || find_verb_on(_this = location, pc, &vh) || find_verb_on(_this = pc->dobj, pc, &vh) || find_verb_on(_this = pc->iobj, pc, &vh) || (valid(location) && !server_int_option("player_huh", PLAYER_HUH) && (vh = db_find_callable_verb(Var::new_obj(_this = location), "huh"), vh.ptr)) || (valid(tq->player) && server_int_option("player_huh", PLAYER_HUH) && (vh = db_find_callable_verb(Var::new_obj(_this = tq->player), "huh"), vh.ptr)))
            {
...
#define TASK_CO_TABLE(DEFINE, tq, value, _)                                                                             \
    ...
    DEFINE(disable - oob, _, TYPE_INT, num,                                                                             \
           ...
           {                                                                                                            \
               ...
               /* Anything to be done? */                                                                               \
               if (!tq->disable_oob && tq->first_input && (tq->first_itail->next || tq->first_input->kind == TASK_OOB)) \
                   ensure_usage(tq);                                                                                    \
           })                                                                                                           \
                                                                                                                        \
...
int tasks_set_connection_option(task_queue q, const char *option, Var value)
{
    CONNECTION_OPTION_SET(TASK_CO_TABLE, (tqueue *)q.ptr, option, value);
}

int tasks_connection_option(task_queue q, const char *option, Var *value)
{
    CONNECTION_OPTION_GET(TASK_CO_TABLE, (tqueue *)q.ptr, option, value);
}

Var tasks_connection_options(task_queue q, Var list)
{
...
enqueue_input_task(tqueue *tq, const char *input, int at_front, int binary, bool is_telnet)
{
    ...
    if (binary)
        ...
        t->kind = TASK_OOB;
    else if (oob_quote_prefix_length > 0 && strncmp(oob_quote_prefix, input, oob_quote_prefix_length) == 0)
        t->kind = TASK_QUOTED;
    else if (sizeof(oob_prefix) > 1 && strncmp(oob_prefix, input, sizeof(oob_prefix) - 1) == 0)
        t->kind = TASK_OOB;
    else
        t->kind = TASK_INBAND;
    ...
    t->t.input.next_itail = nullptr;
    if (at_front && tq->first_input)
    { /* if nothing there, front == back */
    ...
    {
        if (tq->first_input && (((*(tq->last_itail))->kind == TASK_OOB) != (t->kind == TASK_OOB)))
    ...
    if (!tq->hold_input || tq->reading || (!tq->disable_oob && t->kind == TASK_OOB))
        ensure_usage(tq);
...
void task_suspend_input(task_queue q)
{
    tqueue *tq = (tqueue *)q.ptr;
...
void new_input_task(task_queue q, const char *input, int binary, bool out_of_band)
{
    tqueue *tq = (tqueue *)q.ptr;
...
int check_user_task_limit(Objid user)
{
    tqueue *tq = find_tqueue(user, 0);
    int limit = -1;
    Var v;

    if (valid(user) && db_find_property(Var::new_obj(user), "queued_task_limit", &v).ptr && v.type == TYPE_INT)
        limit = v.v.num;
...
void resume_task(vm the_vm, Var value)
{
    task *t = (task *)mymalloc(sizeof(task), M_TASK);
...
Var read_input_now(Objid connection)
{
...
int last_input_task_id(Objid player)
{
    tqueue *tq = find_tqueue(player, 0);
...
void register_task_queue(task_enumerator enumerator)
{
                          ...
                          const char **message, const char **vname)
{
    ...
    *vname = verbref + (colon - copy) + 1;
    ...
    else if (!db_verb_allows(h, player, VF_WRITE) || (server_flag_option("protect_set_verb_code", 0) && !is_wizard(player)))
    {
...
bf_queue_info(Var arglist, Byte next, void *vdata, Objid progr)
{
    ...
    else if (is_wizard(progr))
    {
        ...
        else
        {
            ...
            res = mapinsert(res, var_ref(queue_vm), tq->reading ? Var::new_int(tq->reading_vm->task_id) : Var::new_int(0));
        }
    }
    else
    {
...
bf_queued_tasks(Var arglist, Byte next, void *vdata, Objid progr)
{
    ...
    for (tq = active_tqueues; tq; tq = tq->next)
    {
        ...
        for (t = tq->first_bg; t; t = t->next)
            if (show_all || (t->kind == TASK_FORKED
    ...
    else
    {
        ...
        for (tq = active_tqueues; tq; tq = tq->next)
        {
            ...
            for (t = tq->first_bg; t; t = t->next)
                if (t->kind == TASK_FORKED && (show_all || t->t.forked.a.progr == progr))
                    tasks.v.list[i++] = list_for_forked_task(t->t.forked,
                                                             progr, include_variables);
                else if (t->kind == TASK_SUSPENDED && (show_all || progr_of_cur_verb(t->t.suspended.the_vm) == progr))
        ...
        for (t = waiting_tasks; t; t = t->next)
        {
            if (t->kind == TASK_FORKED && (show_all ||
                ...
                tasks.v.list[i++] = list_for_forked_task(t->t.forked,
                                                         progr, include_variables);
            else if (t->kind == TASK_SUSPENDED && (progr_of_cur_verb(t->t.suspended.the_vm) == progr || show_all))
                tasks.v.list[i++] = list_for_suspended_task(t->t.suspended,
                                                            progr, include_variables);
        }
...
kill_task(int id, Objid owner)
{
    ...
    for (tt = &waiting_tasks; *tt; tt = &((*tt)->next))
    {
        ...
        else if (t->kind == TASK_SUSPENDED && t->t.suspended.the_vm->task_id == id)
    ...
    for (tq = active_tqueues; tq; tq = tq->next)
    {
        ...
        for (tt = &(tq->first_bg); *tt; tt = &((*tt)->next))
        {
            ...
            if ((t->kind == TASK_FORKED && t->t.forked.id == id) || (t->kind == TASK_SUSPENDED && t->t.suspended.the_vm->task_id == id))
            {
                if (!is_wizard(owner) && owner != tq->player)
                    return E_PERM;
                *tt = t->next;
...
{ /* (conn, string [, at_front]) */
    ...
    int at_front = (arglist.v.list[0].v.num > 2 && is_true(arglist.v.list[3]));
    tqueue *tq;
...
{ /* (conn [, show_messages]) */
    Objid conn = arglist.v.list[1].v.obj;
    int show_messages = (arglist.v.list[0].v.num > 1 && is_true(arglist.v.list[2]));
    tqueue *tq;
...
bf_task_valid(Var arglist, Byte next, void *vdata, Objid progr)
{
    ...
    if (the_vm)
    {
        Objid owner = progr_of_cur_verb(the_vm);
