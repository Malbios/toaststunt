...
void print_error_backtrace(const char *msg, void (*output)(const char *)) {
    int t;
    Stream *str;
            ...
            if (why != FIN_ABORT && v.type == TYPE_FINALLY) {
                ...
                *(a->top_rt_stack++) = value;
                ...
                for (vv = new_top; vv < a->top_rt_stack; vv += 2) {
                    if (!found && (vv->type != TYPE_LIST || ismember(code, *vv, 0))) {
                        found = 1;
                        v = *(vv + 1);
                        if (v.type != TYPE_INT)
...
Var make_rt_var_map(Var *rt_env, const char **var_names, unsigned size) {
    Var rt_vars = new_map();
    int i;
...
void free_activation(activation *ap, char data_too) {
    Var *i;
...
static enum outcome
run(char raise, enum error resumption_error, Var *result) { /* runs the_vm */
    ...
    (bv += nb,                                      \
     (nb == 1                                       \
          ...
          : (nb == 2                                \
                 ...
                 : (((unsigned)bv[-4] << 24) + ((unsigned)bv[-3] << 16) + ((unsigned)bv[-2] << 8) + bv[-1]))))
    ...
    next_opcode:
        ...
        switch (op) {
            ...
            case OP_FOR_RANGE: {
                ...
                if ((to.type != TYPE_INT && to.type != TYPE_OBJ) || to.type != from.type) {
                ...
                if (list.type == TYPE_WAIF) {
                    ...
                    if ((list.type != TYPE_LIST && list.type != TYPE_STR &&
                        ...
                        ((list.type == TYPE_LIST || list.type == TYPE_STR) &&
                         index.type != TYPE_INT) ||
                        (list.type == TYPE_MAP && (index.is_collection() && TYPE_ANON != index.type)) || (list.type == TYPE_STR && value.type != TYPE_STR)) {
                    free_var(value);
                    free_var(index);
                    free_var(list);
                    PUSH_ERROR(E_TYPE);
                } else if ((list.type == TYPE_LIST && (index.v.num < 1 || index.v.num > list.v.list[0].v.num /* size */)) || (list.type == TYPE_STR && (index.v.num < 1 || index.v.num > (int)memo_strlen(list.v.str)))) {
                    free_var(value);
                    free_var(index);
                    free_var(list);
                    PUSH_ERROR(E_RANGE);
            ...
            case OP_OR: {
                ...
                lhs = TOP_RT_VALUE;
                if ((op == OP_AND && !is_true(lhs)) || (op == OP_OR && is_true(lhs))) /* short-circuit */
                    JUMP(lab);
                else {
                    free_var(POP());
                }
            ...
            case OP_RANGE_REF: {
                ...
                if (base.type != TYPE_MAP && base.type != TYPE_LIST && base.type != TYPE_STR) {
                    var_type base_type = base.type;
                    free_var(to);
                    free_var(from);
                    free_var(base);
                    PUSH_TYPE_MISMATCH(3, base_type, TYPE_LIST, TYPE_MAP, TYPE_STR);
                } else if (base.type == TYPE_MAP && ((to.is_collection() && TYPE_ANON != to.type) || (from.is_collection() && TYPE_ANON != from.type))) {
                ...
                } else if ((base.type == TYPE_LIST || base.type == TYPE_STR) && (to.type != TYPE_INT || from.type != TYPE_INT)) {
                    ...
                    int len = (base.type == TYPE_STR ? memo_strlen(base.v.str)
                                                     : base.v.list[0].v.num);
                    if (from.v.num <= to.v.num && (from.v.num <= 0 || from.v.num > len || to.v.num <= 0 || to.v.num > len)) {
                        free_var(to);
                        free_var(from);
                        free_var(base);
                        PUSH_ERROR(E_RANGE);
                        ...
                        switch (built_in) {
                            ...
                            case BP_WIZARD:
                                if (!is_wizard(progr))
                                    ...
                                    err = E_INVARG;
                                else if (built_in == BP_WIZARD && !is_true(rhs) != !is_wizard(obj.v.obj)) {
...
#undef MATCH_TYPE
            ...
            case OP_BI_FUNC_CALL: {
                ...
                func_id = READ_BYTES(bv, 2); /* 2 == numbytes of func_id */
            ...
            case OP_EXTENDED: {
                ...
                switch (eop) {
                    case EOP_RANGESET: {
                        ...
                        if ((base.type != TYPE_MAP && base.type != TYPE_LIST && base.type != TYPE_STR) || (base.type != value.type)) {
                            var_type base_type = base.type;
...
#define BASE NEXT_TOP_RT_VALUE
                        ...
                        if (BASE.type != TYPE_STR && BASE.type != TYPE_LIST && BASE.type != TYPE_MAP) {
...
static enum outcome
do_task(Program *prog, int which_vector, Var *result, int is_fg, int do_db_tracebacks) { /* which vector determines the vector for the root_activ.
...
int setup_activ_for_eval(Program *prog) {
    Var *env;
    if (!push_activation())
        return 0;
...
bf_suspend(Var arglist, Byte next, void *vdata, Objid progr) {
    ...
    if (nargs >= 1) {
        seconds = arglist.v.list[1].type == TYPE_INT ? arglist.v.list[1].v.num : arglist.v.list[1].v.fnum;
        secondsp = &seconds;
    } else {
        secondsp = nullptr;
    }
...
bf_yield_if_needed(Var arglist, Byte next, void *vdata, Objid progr) {
    ...
    if (nargs >= 1 && (seconds < 0 || min_ticks <= 0 || min_seconds <= 0 || min_ticks >= server_int_option("fg_ticks", DEFAULT_FG_TICKS) || min_seconds >= server_int_option("fg_seconds", DEFAULT_FG_SECONDS)))
        return make_error_pack(E_INVARG);
...
static package
bf_read(Var arglist, Byte next, void *vdata, Objid progr) { /* ([object [, non_blocking]]) */
    ...
    if (argc >= 1) {
        if (!is_wizard(progr) && (!valid(connection) || progr != db_object_owner(connection)))
            return make_error_pack(E_PERM);
    } else {
        if (!is_wizard(progr) || last_input_task_id(connection) != current_task_id)
            return make_error_pack(E_PERM);
    }
...
static package
bf_read_http(Var arglist, Byte next, void *vdata, Objid progr) { /* ("request" | "response" [, object]) */
    int argc = arglist.v.list[0].v.num;
    static Objid connection;
    int request;
...
static package
bf_set_task_perms(Var arglist, Byte next, void *vdata, Objid progr) { /* (player) */
...
void write_activ_as_pi(activation a) {
    Var dummy;
...
int read_activ_as_pi(activation *a) {
...
void free_reordered_rt_env_values(void) {
    free_var(temp_vars);
}

void write_activ(activation a) {
    Var *v;
...
check_pc_validity(Program *prog, int which_vector, unsigned pc) {
     ...
     */
    return (pc < bc->size && (bc->vector[pc - 1] == OP_CALL_VERB || bc->vector[pc - 3] == OP_BI_FUNC_CALL));
}

int read_activ(activation *a, int which_vector) {
    DB_Version version;
    unsigned int v;
    Var *old_rt_env;
